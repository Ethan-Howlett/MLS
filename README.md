...
This script will make an API fetch request to tools.mtc.byu.edu to make a custom search for missionaries that will be in the Provo MTC durning the seminar. The responsed is cleaned and written to the _Raw Data_ sheet.

### How to setup
First, copy the code in the file and paste it into a new script in Excel -> Automate -> New Scipt -> Create in Code Editor. Give it a useful name, like the name of the file. You can then add it to the workbook and replace the old button. Note: the old button will not work anymore because the script that it was linked to was likley deleted. 
...
In the API Key input you will have to get your MTC Tools API key by inspecting Tools. To do so ...

### How to use

## Finding open classrooms (clean-classroom-assignments.ts)
This script will process the classroom assignments report from MTC Tools and return a list of open classrooms based off what day and time the activity is and district temple assignments.
